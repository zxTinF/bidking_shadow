...
└── getlog/               # 核心业务包
    ...
    ├── posterior_estimator.py # 约束后验总价估算
    └── runner.py         # 主运行循环
```
...
Grid 底部显示 **估算总价格**。它使用 `getlog/posterior_estimator.py` 的候选池后验估算，核心思想是：对每个物品先生成“它可能是哪几个物品”的候选池，再给每个候选一个概率权重，最后把所有物品的候选分布合成总价分布。

手动在 grid 上画出的“有物品”默认视作 **Q5/Q6 高品质范围**。未指定金/红时，它会直接在该形状对应的 Q5/Q6 候选池上估算。若随后手动设为金/红或确认候选，估算总价格会按明确约束刷新。

输入区的 **金个数** 用于记录道具得到的“整局金色 Q5 物品件数”。点击「确认」后，估算会用这个数重新约束未定的 Q5/Q6 物品：

- grid 上已经确定为金的物品会先占用金件数名额。
- grid 上已经确定为红的物品不会占用金件数名额。
- 仍未指定金/红的手动画框，会在 Q5/Q6 之间分配，直到总金件数等于输入值。
- 如果当前 grid 已知金数已经超过输入值，或未定 Q5/Q6 数量不足以补到输入值，则认为约束矛盾，估算总价格会变成 `0`。
- 点击「重置」会清空输入区所有金色约束，并让估算恢复为只依赖当前日志、手动标记和候选池。
- 点击「尝试填充」会清除上一次自动填充生成的框，保留你手动画的框，然后把第 4 回合后仍为空的区域按 Q5/Q6 候选物品形状尝试铺满。程序会用左上、右下、按列、中心、边缘等不同扫描方向生成方案并去重；方案选择优先保留彼此差异更大的全局填法，再按块数少、大块多、候选权重排序；只反复点击「尝试填充」会按方案顺序切换。为避免点击时卡顿，自动填充不会逐方案计算估价，也不会弹出填充估价表。若已输入金个数/金总格/金均格，程序会尽量把自动填充框标成金或红来满足约束；若没有足够约束，则保持金/红未定。
...
5. 如果这个物品有已知价格但没有精确 `ItemCid`，会把价格作为软约束。
   候选价格越接近已知价格，权重越高；偏离越大，权重越低。这样做是为了避免 OCR 或日志不完整时把后验概率直接压成 0。
...
2. 用固定随机种子的 Monte Carlo 采样合成总价分布。
   当前 grid 中使用 `2048` 次采样；固定种子保证同一个局面重复刷新时结果稳定。

3. 每次采样会从每个物品的候选分布中按概率抽一个价格，然后相加得到一次总价样本。

4. 对所有总价样本排序，取中位数 `q50`。

5. 同时计算所有单物品分布带来的总均值和总方差，得到变异系数：
...
```text
估算总价格 = q50 * (1 + min(0.25, cv * 0.15))
```

这样做的目的：

- `q50` 比均值更抗极端红色高价物品的长尾影响。
- `cv` 反映当前局面不确定性；不确定性越高，估价略微上修。
- 上修最多 25%，避免高方差局面把估值抬得过猛。

如果输入了 **金个数 / 金总格 / 金均格**，采样时会被条件化为“抽到的 Q5 件数和 Q5 总格数必须满足输入推导出的组合”。实现上不是简单拒绝采样，而是先用动态规划算出每一步还能补足目标 `(金件数, 金总格)` 的概率，再按这个条件分布采样，避免大量样本因为约束不匹配而浪费。

例子：如果两个未定 Q5/Q6 手动画框分别是 `4` 格和 `2` 格：

- 输入 `金均格 = 4.00`，可能推导出 `(金个数=1, 金总格=4)`，也就是 4 格框为金。
- 输入 `金均格 = 2.00`，可能推导出 `(1, 2)` 或 `(2, 4)`，估算会在这些合法分支中计算。
- 输入 `金个数 = 1` 且 `金均格 = 3.67`，没有任何整数总格能匹配，估算会变成 `0`。

#### 5.6.3 局限

- 当前版本没有做完整的二维全局布局枚举。
- 「尝试填充」是基于 Q5/Q6 候选形状的启发式铺排，不保证等同真实布局；为保持 UI 响应，当前只保留前若干个去重后的方案用于选择。
- 没有用卷积/DP 验证“所有未知物品是否能整体拼成某个合法布局”。
- 默认把各物品候选分布近似独立处理；它没有显式建模“同一局里抽到某个高价物品后其它物品概率应变化”的无放回/相关性。
- 价格软约束是经验模型，不是从游戏机制严格推导。
- 当候选池极大且约束很少时，估算仍然会比较依赖掉落权重质量。
